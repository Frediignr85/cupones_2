                ...
                <div class="ibox-content">
                    <header>
                        <h3 style="color:#60A5FC;"><i class="fa fa-user-md"></i>
                            <b><?php echo "Administrar Ofertas Descartadas";?></b></h3>
                    </header>
                    <section>
                        <table class="table table-striped table-bordered table-hover" id="editable">
                            ...
                            <tbody>
                                <?php  
                                    if($admin){
                                        $query_pro = "SELECT ofertas.id_oferta, empresas.nombre, ofertas.titulo, ofertas.precio_regular, ofertas.precio_oferta, ofertas.cantidad_limite_cupones, ofertas.ilimitar, ofertas.fecha_inicio, ofertas.fecha_fin FROM ofertas INNER JOIN empresas on empresas.id_empresa = ofertas.id_empresa WHERE ofertas.estado_descartado = 1 AND ofertas.eliminado = 0";
                                    }
                                    else{
                                        $query_pro = "SELECT * FROM ofertas WHERE estado_descartado = 1 AND id_empresa = '".$_SESSION['id_sucursal']."' AND eliminado = 0";
                                    }
                                    ...
                                    while($row = _fetch_array($query_pro)){
                                        ...
                                        <ul class=\"dropdown-menu dropdown-primary\">";
                                           $filename='ver_motivo_descarte.php';
                                            $link=permission_usr($id_user,$filename);
                                            if ($link!='NOT' || $admin=='1' )
                                            $boton .= "<li><a data-toggle='modal' href='ver_motivo_descarte.php?id_oferta=".$row ['id_oferta']."&process=formDelete"."' data-target='#deleteModal' data-refresh='true'><i class=\"fa fa-eye\"></i> Ver Motivo</a></li>";
                                            $filename='recuperar_oferta.php';
                                            $link=permission_usr($id_user,$filename);
                                            if ($link!='NOT' || $admin=='1' )
                                            $boton .= "<li><a data-toggle='modal' href='recuperar_oferta.php?id_oferta=".$row ['id_oferta']."&process=formDelete"."' data-target='#deleteModal' data-refresh='true'><i class=\"fa fa-backward\"></i> Recuperar</a></li>";
