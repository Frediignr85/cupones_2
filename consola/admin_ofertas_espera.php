                ...
                <div class="ibox-content">
                    <header>
                        <h3 style="color:#60A5FC;"><i class="fa fa-user-md"></i>
                            <b><?php echo "Administrar Ofertas en Espera";?></b></h3>
                    </header>
                    <section>
                        <table class="table table-striped table-bordered table-hover" id="editable">
                            ...
                            <tbody>
                                <?php  
                                    if($admin){
                                        $query_pro = "SELECT ofertas.id_oferta, empresas.nombre, ofertas.titulo, ofertas.precio_regular, ofertas.precio_oferta, ofertas.cantidad_limite_cupones, ofertas.ilimitar, ofertas.fecha_inicio, ofertas.fecha_fin FROM ofertas INNER JOIN empresas on empresas.id_empresa = ofertas.id_empresa WHERE ofertas.estado_espera = 1 AND ofertas.eliminado = 0";
                                    }
                                    else{
                                        $query_pro = "SELECT * FROM ofertas WHERE estado_espera = 1 AND id_empresa = '".$_SESSION['id_sucursal']."' AND eliminado = 0";
                                    }
                                    ...
                                    while($row = _fetch_array($query_pro)){
                                        ...
                                        <ul class=\"dropdown-menu dropdown-primary\">";
                                            ...
                                            $filename='rechazar_oferta.php';
                                            $link=permission_usr($id_user,$filename);
                                            if ($admin=='1' )
                                            $boton .= "<li><a data-toggle='modal' href='rechazar_oferta.php?id_oferta=".$row ['id_oferta']."&process=formDelete"."' data-target='#deleteModal' data-refresh='true'><i class=\"fa fa-reply\"></i> Rechazar</a></li>";
