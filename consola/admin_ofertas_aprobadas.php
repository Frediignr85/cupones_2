                ...
                <div class="ibox-content">
                    <header>
                        <h3 style="color:#60A5FC;"><i class="fa fa-user-md"></i>
                            <b><?php echo "Administrar Ofertas Aprobadas";?></b></h3>
                    </header>
                    <section>
                        <table class="table table-striped table-bordered table-hover" id="editable">
                            ...
                            <tbody>
                                <?php  
                                    if($admin){
                                        $query_pro = "SELECT ofertas.id_oferta, empresas.nombre, ofertas.titulo, ofertas.precio_regular, ofertas.precio_oferta, ofertas.cantidad_limite_cupones, ofertas.ilimitar, ofertas.fecha_inicio, ofertas.fecha_fin FROM ofertas INNER JOIN empresas on empresas.id_empresa = ofertas.id_empresa WHERE ofertas.estado_aprobado = 1 AND ofertas.eliminado = 0 AND CURDATE() < fecha_inicio";
                                    }
                                    else{
                                        $query_pro = "SELECT * FROM ofertas WHERE estado_aprobado = 1 AND id_empresa = '".$_SESSION['id_sucursal']."' AND CURDATE() < fecha_inicio";
                                    }
